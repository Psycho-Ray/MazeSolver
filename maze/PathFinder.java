...
public class PathFinder {
	...
	private void dfsRun(int x, int y) {
		...
		if (x>0 && source[x-1][y] != 1) dfsRun(x-1, y);	//Esquerda
		if (y+1 < source[x].length && source[x][y+1] != 1) dfsRun(x, y+1);	//Baixo
		if (x+1 < source.length && source[x+1][y] != 1)	dfsRun(x+1, y);	//Direita
		if (y>0 && source[x][y-1] != 1) dfsRun(x, y+1);	//Cima
	...
	private void bfsMark(LinkedList<Point> queue, Point now, Point next) {
		//Adiciona à fila os pixels adjacentes a "now", marca-os como visitados, pinta-os de vermelho e define seus antecessores
	...
	/* A* */
	public ArrayList<LinkedList<Point>> AStar(byte[][] source, Point entrance, int nExits) {
		//Inicialização
		footPrint = new LinkedList<Point>();
		paths = new ArrayList<LinkedList<Point>>(nExits);
		used = new boolean[source.length][];
		...
		set.add(new StarCell(entrance, 0));
		...
		while (!set.isEmpty()) {
			...
			//Esquerda
			if (now.x > 0 && source[now.x - 1][now.y] != 1 && !used[now.x - 1][now.y]) {
				next = new StarCell(now.x - 1, now.y, now.d + 1);
			...
			//Cima
			if (now.y > 0 && source[now.x][now.y - 1] != 1 && !used[now.x][now.y - 1]) {
				next = new StarCell(now.x, now.y - 1, now.d+1);
				AStarMark(set, now, next);
			}
		}
